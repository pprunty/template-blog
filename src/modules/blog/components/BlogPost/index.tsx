...
const BlogPost: React.FC<BlogPostProps> = ({ post, index }) => (
  <Link
    ...
    prefetch={index < 6}
  >
    <div className="flex items-center transition-all ease-in-out border-b border-[#313131] dark:border-[#f0f0f0] sm:border-2 sm:border-gray-200 dark:sm:border-[#313131] sm:hover:border-gray-500 dark:sm:hover:border-gray-400 active:opacity-80 active:scale-98 py-4 sm:py-4 sm:px-4 relative">
        ...
        {post.keywords && (
          <div className="mt-2 flex gap-2 flex-wrap">
            {post.keywords.slice(0, 4).map((tag, index) => (
              <span
                key={index}
                className="text-[11px] border font-mono border-gray-400 dark:border-[#999999] text-gray-700 dark:text-[#999999] px-1"
              >
                {tag}
              </span>
            ))}
